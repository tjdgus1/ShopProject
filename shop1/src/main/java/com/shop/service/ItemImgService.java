...
public class ItemImgService {
    ...
    public void updateItemImg(Long itemImgId, MultipartFile itemImgFile) throws Exception{
        if(!itemImgFile.isEmpty()){ //상품의 이미지를 수정한 경우 상품 이미지 업데이트
            ...
            // 기존에 등록되 상품 이미지 파일이 있는경우 파일 삭제
            if(!StringUtils.isEmpty(savedItemImg.getImgName())){ //문자열이 비어있는지 확인
                fileService.deleteFile(itemImgLocation+"/"+ savedItemImg.getImgName()); //비어있으면 지운다
            }
